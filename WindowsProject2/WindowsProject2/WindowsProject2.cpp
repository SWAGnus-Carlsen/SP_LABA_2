...
const int MAX_STRINGS = numberOfColumns * numberOfRows;
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow)
{
    ...
    ifstream in("C:\\Users\\USER\\Downloads\\SP_LABA_2\\gg.txt");
    string buffer;
    int i = 0;
    if (in.is_open()) {
        while (!in.eof()) {
...
{
    int columnWidth = windowWidth / numberOfColumns;
...
void DrawTable(HDC currDC, int windowWidth, int windowHeight) {
    ...
    while (i < MAX_STRINGS) {
        ...
        int maxTextHeight = 0;  for (int j = 0; (j < numberOfColumns) && (i < MAX_STRINGS); j++)
        {
